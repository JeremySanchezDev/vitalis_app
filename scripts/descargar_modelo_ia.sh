...
#
# El fichero no se versiona (ver .gitignore): pesa ~520 MB y GitHub rechaza
# archivos de más de 100 MB en un commit normal.
set -euo pipefail

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/assets/modelo_ia"
ARCHIVO="$DIR/qwen2.5-0.5b-instruct-q8.task"
...
echo "Descargando modelo de IA local (~520 MB)…"
