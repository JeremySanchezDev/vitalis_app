...
| ADR-03 | **Híbrido**: reglas para plan/agua/entreno + modelo real (Qwen2.5-0.5B-Instruct, empaquetado en la app) para la conversación libre | Lo estructurado nunca lo inventa un LLM (RF-25); la conversación libre sí necesita un modelo real para cumplir el encargo explícito de que el asistente converse de verdad. El modelo va empaquetado, no se descarga, para que la app sea offline desde el instante de instalarse. Detalle completo en [12-modelo-ia-local.md](12-modelo-ia-local.md) | Medio: cambiar de modelo o volver a solo-reglas es una implementación nueva de un contrato, pero el `.task` empaquetado (~520 MB) no vive en git |
| ADR-04 | Dictado del **servicio del sistema** con `onDevice: true`, más **voz de salida** con `flutter_tts` | El audio no sale del teléfono (RNF-01). Si el dispositivo no ofrece reconocimiento local, la app se queda solo con texto, que es la alternativa de AC-11. La voz de salida usa el motor de TTS del sistema, también offline: el asistente responde en voz alta sin depender de ningún servicio externo | Bajo: `Voz` y `SintesisVoz` son contratos separados |
